...
interface AttendanceData {
  status: string;
  event_id?: string; // Optional machen mit ?
...
export default function MemberDetailClient({ id }: { id: string }) {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      const historyData = (attendance || []).map((entry: AttendanceData) => {
        // Sicherstellen, dass wir das Event-Objekt greifen (auch wenn es als Array kommt)
        ...
        let goals = 0;
        if (matches) {
           // 'm' wird automatisch durch den Kontext von 'matches' typisiert
           const eventMatches = matches.filter((m) => m.event_id === event.id);
           
           eventMatches.forEach((match) => {
               if (match.goal_scorers && Array.isArray(match.goal_scorers)) {
                  // Typ für den Scorer-Eintrag direkt definieren
                  const scorerEntry = match.goal_scorers.find((s: { member_id: string; goals: number }) => s.member_id === id);
      ...
      .filter((item): item is HistoryItem => item !== null) // Type Guard statt 'any'
