...
type MemberFormState = {
  ...
  birth_date: string; role: "member" | "coach" | "board" | "admin";
  status: "active" | "passive" | "left" | "guest";
  city_of_residence: string; joined_at: string; left_at: string;
  isEditing: boolean; editId: string | null;
};

type AttendanceStats = {
  total: number; active: number; passive: number; absent: number; helper: number;
};

interface YearlyStatRow {
  member_id: string; active_count: number; passive_count: number;
  helper_count: number; absent_count: number; total_count: number;
}
...
}, [showNotification]); // showNotification muss in die Liste
...
const fetchYearlyStats = useCallback(async (year: number) => {
  ...
  const { currentMembers, guestMembers, leftMembers } = useMemo(() => {
    const filtered = members.filter(member =>
      ...
      ((member.email as string) || "").toLowerCase().includes(searchTerm.toLowerCase()) ||
      ((member.city_of_residence as string) || "").toLowerCase().includes(searchTerm.toLowerCase())
    );
...
// Ersetze die Zuweisungen in der Sortierung durch:
...
valB = (b as Record<string, unknown>)[sortConfig.key] as string | number;
      ...
      let result = 0;
      if (typeof valA === 'string' && typeof valB === 'string') result = valA.localeCompare(valB, 'de');
      else result = Number(valA) - Number(valB);
      return sortConfig.direction === 'desc' ? result * -1 : result;
    });

    return {
      currentMembers: filtered.filter(m => m.status === 'active' || m.status === 'passive' || (!m.status || m.status == 'guest')),
      guestMembers: filtered.filter(m => m.status === 'guest'),
      leftMembers: filtered.filter(m => m.status === 'left')
    };
        ...
        {showForm && canEdit && (
          <div className="mb-8 bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700 shadow-lg sticky top-16 z-50 max-h-[80vh] overflow-y-auto border-t-4 border-t-primary-500">
            ...
            <form onSubmit={handleSaveMember} className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <select className="p-2 rounded border dark:bg-slate-900 dark:border-slate-600 dark:text-white" value={newMember.role} onChange={e => setNewMember({...newMember, role: (e.target.value as MemberFormState["role"])})}>
