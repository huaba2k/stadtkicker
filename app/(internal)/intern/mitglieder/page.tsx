...
import { FaUserPlus, FaEdit, FaTrash, FaSave, FaTimes, FaUsers, FaChartBar, FaFilePdf, FaFilter, FaSort, FaSortUp, FaSortDown, FaUserSlash, FaChevronDown, FaChevronUp } from "react-icons/fa";
...
export default function MitgliederPage() {
  ...
  // State für das Einklappen der Ausgetretenen
  ...
  // --- FILTERN & SPLITTEN ---
  const { currentMembers, leftMembers } = useMemo(() => {
    let filtered = members.filter(member =>
      ...
      (member.birth_date || "").includes(searchTerm.toLowerCase()) ||
      (member.joined_at || "").includes(searchTerm.toLowerCase())
    );
  ...
  const handleSaveMember = async (e: React.FormEvent) => {
    ...
    const dataToSave = {
      ...
      phone: newMember.phone || null, birth_date: newMember.birth_date || null, joined_at: newMember.joined_at || null,
  ...
  const renderTable = (data: Member[], title: string, icon: React.ReactNode) => (
                ...
                {data.map(m => {
                    ...
                    <tr key={m.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
                      <td className="px-6 py-4 font-medium text-slate-900 dark:text-white">
                        ...
                        {m.joined_at && <div className="text-[10px] text-slate-400 font-normal">seit {new Date(m.joined_at).getFullYear()}</div>}
                      </td>
    ...
    <div className="max-w-full mx-auto p-4 sm:p-8 print:p-0">
      ...
      <section id="statistics-report" className="hidden print:block bg-white p-8">
          ...
          <table className="w-full text-sm border-collapse border border-slate-300">
              <thead><tr className="bg-slate-100"><th className="border p-2 text-left">Name</th><th className="border p-2 text-center">Gesamt</th><th className="border p-2 text-center">Aktiv</th><th className="border p-2 text-center">Passiv</th><th className="border p-2 text-center">Helfer</th><th className="border p-2 text-center">Abwesend</th></tr></thead>
              <tbody>{[...currentMembers, ...leftMembers].sort((a,b) => (yearlyAttendance[b.id]?.total||0) - (yearlyAttendance[a.id]?.total||0)).map((m) => (
                      <tr key={m.id} className="break-inside-avoid"><td className="border p-2">{m.last_name}, {m.first_name}</td><td className="border p-2 text-center font-bold">{yearlyAttendance[m.id]?.total || 0}</td><td className="border p-2 text-center">{yearlyAttendance[m.id]?.active || 0}</td><td className="border p-2 text-center text-slate-500">{yearlyAttendance[m.id]?.passive || 0}</td><td className="border p-2 text-center text-purple-600">{yearlyAttendance[m.id]?.helper || 0}</td><td className="border p-2 text-center text-red-500">{yearlyAttendance[m.id]?.absent || 0}</td></tr>
              ))}</tbody>
          </table>
      </section>
      ...
      <div className="print:hidden">
        ...
        {showForm && canEdit && (
          <div className="mb-8 bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700 shadow-lg sticky top-20 z-40 max-h-[80vh] overflow-y-auto">
            ...
            <form onSubmit={handleSaveMember} className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <select className="p-2 rounded border dark:bg-slate-900 dark:border-slate-600 dark:text-white" value={newMember.status} onChange={e => setNewMember({...newMember, status: e.target.value as any})}><option value="active">Aktiv</option><option value="passive">Passiv</option><option value="left">Ausgetreten</option></select>
              <div className="md:col-span-3 flex justify-end gap-2 mt-2"><button type="button" onClick={() => setShowForm(false)} className="px-4 py-2 text-slate-500 hover:bg-slate-100 rounded flex items-center gap-1"><FaTimes /> Abbrechen</button><button type="submit" className="px-6 py-2 bg-primary-600 text-white rounded hover:bg-primary-700 flex items-center gap-1" disabled={loading}><FaSave /> Speichern</button></div>
            </form>
          </div>
        )}
        ...
        {/* TABELLE 1: Aktive & Passive Mitglieder */}
        {loading ? <p className="text-center p-12">Lade...</p> : renderTable(currentMembers, "Mitgliederliste (Aktiv & Passiv)", <FaUsers/>)}

        {/* TABELLE 2: Ausgetretene (Einklappbar) */}
        {!loading && leftMembers.length > 0 && (
            <div className="mt-12 border-t border-slate-200 dark:border-slate-800 pt-8">
                <button onClick={() => setShowLeftMembers(!showLeftMembers)} className="flex items-center gap-2 mx-auto text-slate-500 hover:text-primary-600 font-medium transition-colors mb-6 px-6 py-2 rounded-full bg-slate-100 dark:bg-slate-800">
                    ...
                    {showLeftMembers ? "Ausgetretene ausblenden" : `Ausgetretene anzeigen (${leftMembers.length})`}
                </button>
                {showLeftMembers && <div className="animate-in fade-in slide-in-from-top-4 duration-300">{renderTable(leftMembers, "Ausgetretene Mitglieder", <FaUserSlash className="text-red-400"/>)}</div>}
            </div>
        )}
