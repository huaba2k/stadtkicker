...
export default function BerichtePage() {
  ...
  useEffect(() => {
    const loadReportData = async () => {
      ...
      // 1. Events
      const { data: events } = await supabase.from('events').select('id, title, start_time, category').gte('start_time', startDate).lte('start_time', endDate).order('start_time');
      const eventIds = events?.map(e => e.id) || [];
      const matchEventIds = events?.filter(e => e.category === 'match').map(e => e.id) || [];

      // Zusammenkünfte (Alles außer Schafkopf)
      const gatheringsCount = events?.filter(e => e.category !== 'schafkopf').length || 0;
      setTotalGatherings(gatheringsCount);

      // 2. Matches
      const { data: matches } = await supabase.from('match_results').select('*').in('event_id', matchEventIds);

      // 3. Mitglieder
      const { data: allMembers } = await supabase.from('members').select('*').eq('is_hidden', false).order('last_name', { ascending: true });
      const memberList = (allMembers as Member[]) || [];

      // 4. Anwesenheit
      const { data: attendance } = await supabase.from('attendance').select('member_id, status').in('event_id', eventIds).in('status', ['active', 'passive', 'helper']);
      ...
      if (matches) {
        ...
        const sortedMatches = matches.map(m => {
            ...
            return { 
                ...m, 
                date: evt ? new Date(evt.start_time) : new Date(),
                eventName: evt ? evt.title : "Unbekanntes Spiel"
            };
    ...
    <div className="max-w-5xl mx-auto p-4 sm:p-8">
      <div className="flex flex-col md:flex-row justify-between items-center mb-8 print:hidden">
        ...
        <div className="flex items-center gap-4">
          ...
          <button onClick={handlePrint} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded flex items-center gap-2 transition-colors shadow-sm"><FaPrint size={16} /> Als PDF drucken</button>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 p-8 sm:p-12 rounded-xl shadow-xl print:shadow-none print:p-0 text-slate-900 dark:text-slate-100 border border-slate-200 dark:border-slate-800 print:border-0 print:text-black print:bg-white">
        ...
        <section className="mb-8 text-center break-inside-avoid">
            <div className="inline-block p-6 bg-slate-50 dark:bg-slate-800 rounded-xl border-2 border-slate-200 dark:border-slate-700 print:border-slate-400">
                 ...
                 <span className="text-sm uppercase font-bold text-slate-500 flex items-center justify-center gap-2">
                    <FaUsers className="w-5 h-5" /> Zusammenkünfte (Gesamt)
                 </span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 print:grid-cols-2 print:gap-8">
           ...
           <section className="break-inside-avoid">
              <h3 className="text-xl font-bold mb-4 flex items-center gap-2 border-b border-slate-200 dark:border-slate-700 pb-2 print:border-slate-300"><FaChartPie className="text-blue-600"/> Sportliche Bilanz</h3>
              ...
              <div className="text-center p-4 bg-slate-100 dark:bg-slate-800 rounded-lg print:border print:bg-transparent"><span className="font-bold">Torverhältnis:</span> {matchStats.goalsFor} : {matchStats.goalsAgainst}</div>
           </section>

           {/* Ehrungen */}
           <section className="break-inside-avoid">
              <h3 className="text-xl font-bold mb-4 flex items-center gap-2 border-b border-slate-200 dark:border-slate-700 pb-2 print:border-slate-300"><FaMedal className="text-amber-500"/> Ehrungen & Jubiläen</h3>
              {jubilees.length === 0 ? <p className="text-slate-500 italic">Keine Jubiläen in diesem Jahr.</p> : (
                 <ul className="space-y-3">{jubilees.map((j, i) => (
                    <li key={i} className="flex justify-between items-center p-2 bg-amber-50 dark:bg-amber-900/20 rounded border border-amber-100 dark:border-amber-900/50 print:border-slate-300 print:bg-transparent">
                       <span className="font-bold dark:text-amber-100">{j.member.first_name} {j.member.last_name}</span><span className="text-sm font-mono bg-amber-200 dark:bg-amber-800 text-amber-900 dark:text-amber-100 px-2 py-1 rounded print:bg-slate-200 print:text-black">{j.years} Jahre</span>
                    </li>
                 ))}</ul>
              )}
           </section>
           ...
           {/* Torschützen */}
           <section className="break-inside-avoid">
              <h3 className="text-xl font-bold mb-4 flex items-center gap-2 border-b border-slate-200 dark:border-slate-700 pb-2 print:border-slate-300"><FaTrophy className="text-yellow-500"/> Top Torschützen</h3>
              <ol className="space-y-2">{topScorers.map((s, i) => (
                 <li key={i} className="flex justify-between items-center border-b border-slate-100 dark:border-slate-800 pb-1 last:border-0"><span className="flex items-center gap-2"><span className={`w-5 h-5 flex items-center justify-center text-xs rounded-full ${i===0?'bg-yellow-400 text-white print:text-black print:bg-slate-200':i===1?'bg-slate-300 text-white print:text-black print:bg-slate-200':'bg-orange-400 text-white print:text-black print:bg-slate-200'}`}>{i+1}</span>{s.name}</span><span className="font-bold">{s.goals}</span></li>
              ))}{topScorers.length === 0 && <p className="text-slate-500 italic">Keine Tore verzeichnet.</p>}</ol>
           </section>

           {/* Anwesenheit */}
           <section className="break-inside-avoid">
              <h3 className="text-xl font-bold mb-4 flex items-center gap-2 border-b border-slate-200 dark:border-slate-700 pb-2 print:border-slate-300"><FaCalendarCheck className="text-green-600"/> Meiste Anwesenheit</h3>
              <ol className="space-y-2">{topAttendance.map((s, i) => (
                 <li key={i} className="flex justify-between items-center border-b border-slate-100 dark:border-slate-800 pb-1 last:border-0"><span className="flex items-center gap-2"><span className="text-xs font-bold text-slate-400">{i+1}.</span>{s.name}</span><span className="font-mono text-slate-600 dark:text-slate-400">{s.count}x</span></li>
              ))}{topAttendance.length === 0 && <p className="text-slate-500 italic">Keine Daten verfügbar.</p>}</ol>
           </section>
        </div>
                ...
                {roundBirthdays.length === 0 ? <p className="text-sm text-slate-400">Keine runden Geburtstage.</p> : (
                    <ul className="space-y-3">{roundBirthdays.map((b, i) => (
                        <li key={i} className="flex justify-between items-center p-2 bg-pink-50 dark:bg-pink-900/20 rounded border border-pink-100 dark:border-pink-900/50 print:border-slate-300 print:bg-transparent">
                            <span className="dark:text-pink-100">{b.member.first_name} {b.member.last_name} ({b.date.toLocaleDateString('de-DE', {day: '2-digit', month:'2-digit'})})</span><span className="text-xs font-bold bg-pink-200 text-pink-800 px-2 py-1 rounded print:bg-slate-200 print:text-black">{b.age}.</span>
                        </li>
                    ))}</ul>
                )}
             </section>
             ...
             <section className="break-inside-avoid">
                ...
                <div className="space-y-6">
                    <div>
                        ...
                        {newMembers.length === 0 ? <span className="text-xs text-slate-400">-</span> : (<ul className="space-y-2">{newMembers.map(m => (<li key={m.id} className="flex justify-between text-sm border-b border-dotted border-slate-200 dark:border-slate-800 pb-1"><span>{m.first_name} {m.last_name}</span><span className="text-slate-500 font-mono text-xs">{m.joined_at ? new Date(m.joined_at).toLocaleDateString('de-DE') : ''}</span></li>))}</ul>)}
                    </div>
                    <div>
                        <h4 className="text-sm font-bold text-red-600 dark:text-red-400 mb-2 flex gap-2 items-center"><FaUserMinus/> Abgänge</h4>
                        {leftMembers.length === 0 ? <span className="text-xs text-slate-400">-</span> : (<ul className="space-y-2">{leftMembers.map(m => (<li key={m.id} className="flex justify-between text-sm border-b border-dotted border-slate-200 dark:border-slate-800 pb-1"><span>{m.first_name} {m.last_name}</span><span className="text-slate-500 font-mono text-xs">{m.left_at ? new Date(m.left_at).toLocaleDateString('de-DE') : ''}</span></li>))}</ul>)}
                    </div>
                </div>
             </section>
        </div>
        ...
        <section className="mt-12 break-inside-avoid">
            <h3 className="text-xl font-bold mb-6 flex items-center gap-3 border-b border-slate-200 dark:border-slate-800 pb-2"><FaListUl className="text-slate-600"/> Spiele im Detail</h3>
            {matchList.length === 0 ? <p className="italic text-slate-500">Keine Spiele erfasst.</p> : (
                <table className="w-full text-sm text-left border-collapse">
                    <thead className="bg-slate-100 dark:bg-slate-800 print:bg-slate-200"><tr><th className="p-2">Datum</th><th className="p-2">Gegner / Event</th><th className="p-2 text-center">Erg.</th><th className="p-2">Torschützen</th></tr></thead>
                    <tbody className="divide-y divide-slate-200 dark:divide-slate-800">{matchList.map((m, i) => (
                            <tr key={i} className="break-inside-avoid"><td className="p-2 whitespace-nowrap text-slate-500">{m.date.toLocaleDateString('de-DE')}</td><td className="p-2"><span className="font-bold block">{m.opponent}</span>{m.eventName && !m.eventName.includes('Spiel') && (<span className="text-xs text-slate-400 block uppercase">{m.eventName}</span>)}</td><td className="p-2 text-center font-mono font-bold bg-slate-50 dark:bg-slate-800/50">{m.result}</td><td className="p-2 text-xs text-slate-600 dark:text-slate-400">{m.scorers.join(', ')}</td></tr>
                    ))}</tbody>
                </table>
            )}
        </section>

        <div className="mt-12 text-center text-xs text-slate-400 print:block hidden">Automatisch generierter Bericht via Vereins-App</div>
      </div>
    </div>
  );
}